...
# ---------------------------------------------------------------------
# Based on your Supabase table columns (authoritative via information_schema):
# politician_id (PK)
# scandals_flagged, wealth_declaration_issues, public_projects_completed,
# parliamentary_attendance, international_meetings, party_control_index,
# media_mentions_monthly, legislative_initiatives, independence_index,
# media_positive_events, media_negative_events, updated_at, sentiment_score
CANONICAL_TO_DB_PARAGON_METRICS: Dict[str, str] = {
    "scandals_flagged": "scandals_flagged",
    "wealth_declaration_issues": "wealth_declaration_issues",
    ...
    "media_mentions_monthly": "media_mentions_monthly",
    "legislative_initiatives": "legislative_initiatives",
    "independence_index": "independence_index",
    "media_positive_events": "media_positive_events",
    "media_negative_events": "media_negative_events",
    "sentiment_score": "sentiment_score",
}
...
def _sentiment_float_to_int(v: Any) -> int:
    ...
    DB sentiment_score is int4 storing scaled [-100, 100].
    """
    f = _as_float(v, 0.0)
    f = _clamp(f, -1.0, 1.0)
...
def _sentiment_int_to_float(v: Any) -> float:
    """
    DB sentiment_score is int4 [-100, 100].
    Canonical sentiment_score is float [-1, 1].
    """
    i = _as_int(v, 0)
...
def canonical_to_db_paragon_metrics(canonical: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Also applies scaling for sentiment_score -> sentiment_score (int4 scale).
    """
    out: Dict[str, Any] = {}
...
def db_paragon_metrics_to_canonical(db_row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Also applies scaling for sentiment_score (int4) -> sentiment_score (float).
    """
    out: Dict[str, Any] = {}
